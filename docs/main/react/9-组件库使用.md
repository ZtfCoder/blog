...
const CrudPage = () => {
   ...
   * onChange 表示 切换页面, 以及切换pageSize 时,会调用这个函数,详情可以查看分页组件的props
   * 由于 分页组件的 onChange 函数,需要传递2个参数,所以,我们在修改 fetchData 的时候,给他添加上2个参数
   */
...
type Props = {
  isModalOpen: boolean;
  onFinish: (values:FieldType) => void; // [!code focus]
