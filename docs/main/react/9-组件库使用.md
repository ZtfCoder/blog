...
type Props = {
  isModalOpen: boolean;
  //注意这个函数,这个函数是Form 表单组件绑定的函数,Form组件会在你点击提交确认的时候,调用这个函数,并把表单的数据传给这个函数  // [!code focus]
  onFinish: (values:FieldType) => void; //注意这个,这个values 就是表单的数据,FieldType就是他的类型  // [!code focus]
  onFinishFailed: (errorInfo:any) => void;
  onCancel: () => void;
};

// 表单的字段的类型 // [!code focus]
type FieldType = { // [!code focus]
  id?:number // [!code focus]
  name: string; // [!code focus]
  age: string; // [!code focus]
}; // [!code focus]
...
const ModalForm = (props: Props) => {
  ...
  return (
    <Modal
      ...
      onOk={handleOk}
      onCancel={handleCancel}
      footer={null} // 主要这里,是新增的一个代码,我们这里不需要弹窗的底部确认按钮,只需要表单的按钮就可以了  // [!code focus]
    >
      <Form
        name="basic"
        labelCol={{ span: 8 }}
...
const CrudPage = () => {
  ...
  const onFinish = async (data) => { // [!code focus]
    const result = await axios.post("https://frp-end.top:49556/user/save", data); // [!code focus]
    if (result.status === 200 && result.data.code === 200) { // [!code focus]
      setOpen(false); // [!code focus]
      message.success("添加成功"); // [!code focus]
      // 刷新表格 // [!code focus]
      await fetchData(1, 10); // [!code focus]
    }else{ // [!code focus]
      message.error("添加失败"); // [!code focus]
    }  // [!code focus]
  };  // [!code focus]
...
type Props = {
  ...
  onFinishFailed: (errorInfo:any) => void;
  onCancel: () => void;
  // 看这里,主要新增变动,values 表示我们回显的数据,数据由父组件传入    // [!code focus]
  values?:FieldType  // [!code focus]
};
...
const EditModalForm = (props: Props) => {
  ...
  const { isModalOpen, onFinish, onFinishFailed,onCancel,values } = props; // [!code focus]
  ...
  return (
    ...
    >
      <Form
        ...
        wrapperCol={{ span: 16 }}
        style={{ maxWidth: 600 }}
        // 只要values的对象名称和表单的name值匹配,则可以回显到对应的表单中,也就是Form.Item 中的name的值 // [!code focus]
        // 这里 initialValues 用来回显数据的,我们传入的数据是values // [!code focus]
        initialValues={values} // [!code focus]
        onFinish={onFinish}
        onFinishFailed={onFinishFailed}
        autoComplete="off"
      >
...
const CrudPage = () => {
  ...
  const [openEdit, setOpenEdit] = useState(false);// 新增一个控制变量,来控制是否弹窗修改弹窗  // [!code focus]
  const [editUserInfo, setEditUserInfo] = useState<FieldType>();// 用来存储编辑弹窗的数据回显的  // [!code focus]
  ...
  //  编辑弹窗的提交事件 // [!code focus]
  const onFinishEdit = async (data) => { // [!code focus]
    const result = await axios.post( "https://frp-end.top:49556/user/updateById",data); // [!code focus]
    if (result.status === 200 && result.data.code === 200) { // [!code focus]
      setOpenEdit(false); // [!code focus]
      message.success("修改成功"); // [!code focus]
      // 刷新表格 // [!code focus]
      await fetchData(1, 10); // [!code focus]
    } else { // [!code focus]
      message.error("修改失败"); // [!code focus]
    } // [!code focus]
  }; // [!code focus]
  ...
  //这里是点击修改按钮的的事件 // [!code focus]
  const editUser = (record:FieldType) => { // [!code focus]
    // 设置回显数据 // [!code focus]
    setEditUserInfo(record); // [!code focus]
    // 打开弹窗 // [!code focus]
    setOpenEdit(true); // [!code focus]
  }
  ...
  return (
    <div>
      ...
      />
      {/* 编辑修改弹窗 // [!code focus]  */}
       <EditModalForm // [!code focus]
        values={editUserInfo} // [!code focus]
        isModalOpen={openEdit} // [!code focus]
        onFinish={onFinishEdit} // [!code focus]
        onFinishFailed={onFinishFailed} // [!code focus]
        onCancel={onCancel} // [!code focus]
      />
    </div>
  );
};
...
const EditModalForm = (props: Props) => {
  ...
  return (
    <Modal
      ...
      onCancel={handleCancel}
      footer={null}
      destroyOnClose={true} // [!code focus]
    >
      <Form
        name="basic"
        labelCol={{ span: 8 }}
...
import { Button, message, Popconfirm, Table } from "antd"; //注意这里要引入我们的确认框  // [!code focus]
import axios from "axios";
import { useEffect, useState } from "react";
import ModalForm from "../../components/ModalForm";
import EditModalForm from "../../components/EditModalForm";
...
const CrudPage = () => {
  ...
  // [!code focus]
 const deleted = async (record: FieldType) => { // [!code focus]
    const result =  await axios.post("https://frp-end.top:49556/user/delete", record) // [!code focus]
    if(result.status === 200 && result.data.code === 200){ // [!code focus]
      await fetchData(1, 10); // [!code focus]
      message.success("删除成功") // [!code focus]
    }else{  // [!code focus]
      message.error("删除失败") // [!code focus]
    } // [!code focus]
  }; // [!code focus]
...
在讲文件上传之前,我们需要大概了解下,`Form` 表单组件的运行方式,为什么,我们只需要在`<Form.Item></Form.Item>` 写一个输入框,不需要写其他的事件,也不需要监听输入框的变化并存储,可以看下我们之前写的输入框 有关的代码中,我们是通过,在输入框上,使用`onChange` 事件,把用户输入的值,存储到一个 `useState` 的变量中,并且,在输入框的`input` 的value 属性赋值赋上我们的`useState`
...
````
通过这种方式,我可以存储并获取用户输入的值,但是在`antd` 中,我们并没有写这些代码,原因是因为,`antd` 自动帮我们做了这些事,他其实还是调用了`onChange` 并且赋值给 `value`
...
type UploadFormProps = {
  onChange?: (info: {headPic:string}) => void;
};
...
const UploadForm = (props: UploadFormProps) => {
  ...
  const config: UploadProps = {
    name: "file",
    action: "https://frp-end.top:49556/api/upload",
    onChange(info) {
      ...
      if (info.file.status === "done") {
        if (onChange) {
          onChange({
            headPic: info.file.response?.data?.path
          });
        }
